...
function ($j) {
    ...
    */
    $j.EndecaSearchSuggestor.prototype.composeUrl = function () {
        ...
        searchTerm = encodeURIComponent(this.RemoveSearchInvalidChar(searchTerm));
    ...
    $j.EndecaSearchSuggestor.prototype.generateHtmlContent = function (dimSearchResult) {
        ...
        if (dimSearchResult != null && dimSearchResult.dimensionSearchGroups.length > 0) {
            ...
            for (var i = 0; i < dimSearchGroupList.length; i++) {
                ...
                var dimensionName = dimResultGroup.dimensionName;
                ...
                for (var j = 0; j < dimResultGroup.dimensionSearchValues.length; j++) {
                    ...
                    if (typeof dimResult.properties.SourceId != "undefined")
                    {
                        //exclude production hidden catalogue 1773
                        if(displayName == "Category")
                        {
                            if((dimResult.properties.SourceId.indexOf("1773_") > -1) || (dimResult.properties.SourceId.indexOf("_1773") > -1))
                                continue;
                        }
                    }

                     if (dimensionName == "Keywords"){
                        action = dimResult.siteRootPath + dimResult.contentPath + '?Ntk=All&Ntt=' + encodeURIComponent(text) ;
                        dimResult.count = null;
                     }
                     else
                     {
                        //var strNavigation = dimResult.navigationState.replace("?", ".aspx?");  
                        //if( strNavigation.indexOf('?') == -1)  
                            //strNavigation+=".aspx";                
                        action = dimResult.siteRootPath + dimResult.contentPath + dimResult.navigationState;
                    ...
                    //disable image
                    dimSearchResult.displayImage = false;
                    var _href = this._options.searchUrl + action;
                    if (_href.indexOf('?') == -1) {
                        _href += '?';
                    }
                    else {
                    ...
                    if (dimSearchResult.displayImage) {
                        ...
                        newContent.append('<div class="dimResult"><table><tr><td width="36"><img src="'                                 
                                + imageUrl + '"/></td><td><div class="link"><a href="' + _href + '">'
                                + this.highlightMatched(text) + '</a>' + count + '<div></td></tr></table></div>');
                    }
                    else {
                        newContent.append('<div class="dimResult"><div class="link"><a href="' + _href + '">'                        
                                + this.highlightMatched(text) + '</a>' + count + '<div></div>');
                    }
                }
            }
        }
    ...
    */
    ...
    /**
    * Remove invalid characters from search text.
    */
    $j.EndecaSearchSuggestor.prototype.RemoveSearchInvalidChar = function (text) {
        var validWords;
        validWords = text.match(/[0-9a-zA-ZúÊ‡‚ÁÈËÍÎÓÔÙ˚˘¸ˇÒå∆¿¬«…» ÀŒœ‘€Ÿ‹ü—,.;:""\[\]!@#$%^&*()/\-_+\s]/g);
        var result = "";
        for ( var i = 0; i < validWords.length; i = i + 1 )
        {
            result += validWords[i];
        }
        return result;
