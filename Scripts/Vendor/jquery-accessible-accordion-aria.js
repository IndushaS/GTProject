$(document).ready(function () {
    ...
    if ($accordions.length) { // if there are at least one :)
        ...
        $accordions.each(function (index) {
            var $this = $(this);
            $options = $this.data(),
              $accordions_links = $this.find(".js-accordion__link"),
            ...
            $accordions_links.each(function (index_l) {
                var $that = $(this),
                    $text = $that.text(),
                    $index_link = index_l + 1,
                    $link_span = $('<span>' + $text + "</span>");

                $that.addClass($accordions_prefix_classes + "__link").attr({
                    "id": "accordion" + $index_accordion + "_link" + $index_link
                }).empty();
                $link_span.appendTo($that);
            ...
            $accordions_headers.each(function (index_h) {
                ...
                $accordion_panel.prepend($that.removeClass("js-accordion__header").addClass($accordions_prefix_classes + "__title"));

                $accordion_header = $('<button class="js-accordion__header ' + $accordions_prefix_classes + '__header"><span>' + $text + '</span></button>');
